...
// vector<mutex> entity_mutex;
vector<pthread_mutex_t> entity_mutex;
...
void init() {
	...
	for (int i = 0; i < relationTotal; i++) {
		...
		for (int ii=0; ii<dimension; ii++)
			relv[i][ii] = randn(0, 1.0 / dimension, -6 / sqrt(dimension), 6 / sqrt(dimension));
	}
	...
	for (int i = 0; i < entityTotal; i++) {
		...
		for (int iii=0; iii<dimension; iii++)
			temp[iii] = randn(0, 1.0 / dimension, -6 / sqrt(dimension), 6 / sqrt(dimension));
	...
	for (size_t i = 0; i < entityTotal; i++) {
		...
		sigma[i].push_back(1.0);
	}
	// vector<mutex> vmutex(entityTotal);
	printf("before init mutex\n");
	vector<pthread_mutex_t> vmutex(entityTotal);
	entity_mutex.swap(vmutex);
	for (size_t i = 0; i < entityTotal; i++) {
		pthread_mutex_init(&entity_mutex[i], NULL);
	}
...
double prob(int a, int b, int r) {
	...
	for (size_t ai = 0; ai < factor[a].size(); ai++) {
		for (size_t bi = 0; bi < factor[b].size(); bi++) {
			p += factor[a][ai]*factor[b][bi]*exp(-norm(entv[a][ai],entv[b][bi],relv[r])/(sigma[a][ai]*sigma[a][ai]+sigma[b][bi]*sigma[b][bi]));
		}
	}
	if (isnan(entv[a][0][0])) {
		printf("%i,%i,%i\n", a,r,b);
...
double prob(int a, int b, const vector<double> & r) {
	double p = 0.0;
	for (size_t ai = 0; ai < factor[a].size(); ai++) {
		for (size_t bi = 0; bi < factor[b].size(); bi++) {
			p += factor[a][ai]*factor[b][bi]*exp(-norm(entv[a][ai],entv[b][bi],r)/(sigma[a][ai]*sigma[a][ai]+sigma[b][bi]*sigma[b][bi]));
		}
	}
	return p;
}
...
double loss_gradient(int a, int b, int r, bool sample_type) {
	...
	for (size_t m = 0; m < factor[a].size(); m++) {
		for (size_t n = 0; n < factor[b].size(); n++) {
			smn = factor[a][m]*factor[b][n]*exp(-norm(entv[a][m],entv[b][n],relv[r])/(sigma[a][m]*sigma[a][m]+sigma[b][n]*sigma[b][n]))/(sigma[a][m]*sigma[a][m]+sigma[b][n]*sigma[b][n]);
			x = local_const*norm(entv[a][m],entv[b][n],relv[r])*smn/(sigma[a][m]*sigma[a][m]+sigma[b][n]*sigma[b][n]);
...
void train_kb_mix(int a1, int b1, int r1, int a2, int b2, int r2) {
	// entity_mutex[a1].lock();
	// entity_mutex[b1].lock();
	// entity_mutex[r1].lock();
	// if (a2!=a1) entity_mutex[a2].lock();
	// if (b2!=b1) entity_mutex[b2].lock();
	// if (r2!=r1) entity_mutex[r2].lock();
	pthread_mutex_lock(&entity_mutex[a1]);
	pthread_mutex_lock(&entity_mutex[b1]);
	pthread_mutex_lock(&entity_mutex[r1]);
	if (a2!=a1) pthread_mutex_lock(&entity_mutex[a2]);
	if (b2!=b1) pthread_mutex_lock(&entity_mutex[b2]);
	if (r2!=r1) pthread_mutex_lock(&entity_mutex[r2]);
	printf("lock %i %i %i %i %i %i\n", a1,b1,r1,a2,b2,r2);
	if(prob(a1,b1,r1)/prob(a2,b2,r2)>2.0) return;
	double x = 0.0;
	double smn = 0.0;
	...
	res += fff;
	// cout << ttt << " " << fff << endl;
	res += regular_gradient(a1, b1, r1);
	res += regular_gradient(a2, b2, r2);
    generate(a1, b1, r1);
    // cout << "-----------" << endl;
	// entity_mutex[a1].unlock();
	// entity_mutex[b1].unlock();
	// entity_mutex[r1].unlock();
	// if (a2!=a1) entity_mutex[a2].unlock();
	// if (b2!=b1) entity_mutex[b2].unlock();
	// if (r2!=r1) entity_mutex[r2].unlock();
	pthread_mutex_unlock(&entity_mutex[a1]);
	pthread_mutex_unlock(&entity_mutex[b1]);
	pthread_mutex_unlock(&entity_mutex[r1]);
	if (a2!=a1) pthread_mutex_unlock(&entity_mutex[a2]);
	if (b2!=b1) pthread_mutex_unlock(&entity_mutex[b2]);
	if (r2!=r1) pthread_mutex_unlock(&entity_mutex[r2]);
	printf("unlock %i %i %i %i %i %i\n", a1,b1,r1,a2,b2,r2);
}
...
int main() {
	...
	train_transe(NULL);
	for (size_t i = 0; i < entityTotal; i++) {
		pthread_mutex_destroy(&entity_mutex[i]);
	}
	out_transe();
	return 0;
}
